...
class Scorecard extends CI_Controller 
{
    ...
    public function index()
    {
        ...
        $data += [
            'scores' => $this->scorecard_model->getAll($data['tdc']),
        ];
    ...
    public function exportpdf($start, $end)
    {
        $data = $this->userSession();   
        $data += [
            'scorecard' => $this->scorecard_model->getAll($data['tdc'],$start, $end)
        ];

        $this->load->view('indirect/scorecard/pdf_export', $data);
    }

    public function export($start, $end)
    {
        $data = $this->userSession();
        $export = $this->scorecard_model->getAll($data['tdc'], $start, $end);
